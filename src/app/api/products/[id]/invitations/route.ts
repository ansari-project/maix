...
// POST /api/products/[id]/invitations - Create invitation
export async function POST(
  ...
  try {
    ...
    if (await isEmailAlreadyInvited(data.email, productId)) {
      return NextResponse.json(
        { error: 'Email already has a pending invitation for this product' },
        { status: 409 }
      );
    }
    ...
    if (existingUser) {
      const existingMembership = await prisma.productMember.findUnique({
        where: {
          productId_userId: {
            productId,
      ...
      if (existingMembership) {
        return NextResponse.json(
          { error: 'User is already a member of this product' },
          { status: 409 }
        );
      }
    }
    ...
    console.error('Error creating product invitation:', error);
    return NextResponse.json(
...
// GET /api/products/[id]/invitations - List invitations
export async function GET(
  ...
  try {
    ...
    console.error('Error listing product invitations:', error);
    return NextResponse.json(
      { error: 'Internal server error' },
      { status: 500 }
    );
